...
public class WaylineFileServiceImpl implements IWaylineFileService {
	...
	@Override
	public InputStream getObject(String workspaceId, String waylineId) throws EntityNotFoundException, IOException {
		WaylineFileDTO waylineFileDTO = this.getWaylineByWaylineId(workspaceId, waylineId)
			.orElseThrow(() -> new EntityNotFoundException(waylineId + " does not exist."));
		InputStream object = ossService.getObject(OssConfiguration.bucket, waylineFileDTO.getObjectKey());
		if (object.available() == 0) {
			throw new EntityNotFoundException("The file for waylineId " + waylineId +
				" does not exist in the bucket[" + OssConfiguration.bucket + "].");
		}
		return object;
	}
		...
		try (ZipInputStream unzipFile = new ZipInputStream(file.getInputStream(), StandardCharsets.UTF_8)) {
			...
			while (Objects.nonNull(nextEntry)) {
				boolean isWaylines = (KmzFileProperties.FILE_DIR_FIRST + "/"
					+ KmzFileProperties.FILE_DIR_SECOND_TEMPLATE).equals(nextEntry.getName());
				if (!isWaylines) {
				...
				String type = droneNode.valueOf(
					KmzFileProperties.TAG_WPML_PREFIX + KmzFileProperties.TAG_DRONE_ENUM_VALUE);
				String subType = droneNode.valueOf(
					KmzFileProperties.TAG_WPML_PREFIX + KmzFileProperties.TAG_DRONE_SUB_ENUM_VALUE);
				String payloadType = payloadNode.valueOf(
					KmzFileProperties.TAG_WPML_PREFIX + KmzFileProperties.TAG_PAYLOAD_ENUM_VALUE);
				String payloadSubType = payloadNode.valueOf(
					KmzFileProperties.TAG_WPML_PREFIX + KmzFileProperties.TAG_PAYLOAD_SUB_ENUM_VALUE);
				String templateType = document.valueOf(
				...
				return Optional.of(WaylineFileDTO.builder()
					...
					.payloadModelKeys(List.of(
						String.format("%s-%s-%s", DeviceDomainEnum.PAYLOAD.getVal(), payloadType, payloadSubType)))
					...
					.templateTypes(List.of(
						WaylineTemplateTypeEnum.find(templateType).map(WaylineTemplateTypeEnum::getVal).orElse(-1)))
					.build());
			}
